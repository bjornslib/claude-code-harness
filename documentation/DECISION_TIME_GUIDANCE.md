...
1. **Escalate to CoBuilder via bead status**:
   ```bash
   bd update <id> --status=blocked --notes="worker_failure: <details>"
   ```

2. **Wait for CoBuilder response** - CoBuilder monitors bead statuses for blocked items
...
**Required Action**:
1. Escalate blockers to CoBuilder via bead status:
   ```bash
...
**Recommended Action - Escalate to CoBuilder:**
...
```bash
# Escalate blockers to CoBuilder via bead status update
