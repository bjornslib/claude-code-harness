...
The `validation-test-agent` has been updated to route to these skills:
...
3. Never call acceptance-test skills directly - always through validation-test-agent
...
| File | Changes |
|------|---------|
| `orchestrator-multiagent/WORKFLOWS.md` | Replace `bd close` with validation-test-agent delegation |
...
### Through validation-test-agent (CORRECT)
```python
# ✅ Always route through validation-test-agent
Task(subagent_type="validation-test-agent",
...
Worker completes
    ↓
Orchestrator runs: validation-test-agent --mode=unit (fast check)
    ↓
If pass: validation-test-agent --mode=e2e --prd=PRD-XXX (acceptance tests)
    ↓
validation-test-agent invokes acceptance-test-runner skill
    ↓
If all criteria pass: validation-test-agent closes task with evidence
If any fail: Report to orchestrator for remediation
```

## Completion Checklist

- [x] WORKFLOWS.md - Replace bd close with validation-test-agent
