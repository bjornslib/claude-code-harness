...
def main():
    ...
    if not result.passed:
        ...
        # Promises passed - check orchestrator guidance for orch-* sessions
        if config.is_orchestrator:
            orch_checker = OrchestratorGuidanceChecker(config, paths)
            orch_result = orch_checker.check()
            if not orch_result.passed:
                # Block for orchestrator guidance escalation
                output = {
                    "decision": "block",
                    "reason": orch_result.message
                }
