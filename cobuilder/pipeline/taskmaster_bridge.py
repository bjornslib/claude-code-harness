...
def create_enriched_input(sd_path: str, repomap_context: str, dot_pipeline_context: str = "") -> str:
    ...
    Args:
        ...
        dot_pipeline_context: Optional pipeline context from get_pipeline_context().
            When provided, appends a third section describing pipeline execution state.

    Returns:
        Combined string: SD content + codebase context block + optional pipeline block
    """
...
"""
    ...
    if dot_pipeline_context:
        base += f"""
---

## Pipeline Context (Auto-Generated by CoBuilder)

{dot_pipeline_context}

IMPORTANT for task decomposition:
- Use Pipeline Context to understand current execution state.
- PENDING nodes represent work not yet started — generate tasks for these.
- ACTIVE nodes are in progress — avoid duplicating them.
- VALIDATED nodes are complete — DO NOT generate tasks for these.
"""
...
def run_taskmaster_parse(
    ...
    Args:
        ...
        dot_pipeline_path: Optional path to an existing .dot pipeline file.
            When provided, injects pipeline execution state context into the input.
    """
    input_path = enriched_sd_path
    tmp_file = None

    if repomap_context:
        dot_ctx = get_pipeline_context(dot_pipeline_path) if dot_pipeline_path else ""
        enriched_content = create_enriched_input(enriched_sd_path, repomap_context, dot_ctx)
    ...
    elif dot_pipeline_path:
        dot_ctx = get_pipeline_context(dot_pipeline_path)
        if dot_ctx:
            enriched_content = create_enriched_input(enriched_sd_path, "", dot_ctx)
            tmp_file = tempfile.NamedTemporaryFile(
                mode="w", suffix=".md", delete=False, encoding="utf-8"
            )
            tmp_file.write(enriched_content)
