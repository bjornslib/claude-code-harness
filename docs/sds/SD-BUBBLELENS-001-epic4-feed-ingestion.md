...
```
Extension → POST /api/feeds → Validate (Zod) → Rate Check (Redis) → Auth Check (Clerk)
...
export async function POST(req: Request) {
  ...
  // 2. Get internal user ID from Clerk ID
  const user = await db.user.findUnique({ where: { clerkId: userId } });
  if (!user) return new Response('User not found', { status: 404 });

  // 3. Rate limit check
  const { success, remaining, reset } = await feedRateLimiter.limit(user.id);
  if (!success) {
...
```typescript
// GET /api/feeds - List user's feed snapshots
export async function GET(req: Request) {
  ...
  const total = await db.feedSnapshot.count({ where: { userId: user.id } });
...
```typescript
// GET /api/feeds/[id] - Get single feed with items and video metadata
...
- `feed_snapshots` — one record per capture event
- `feed_items` — one record per video in a capture, linked to snapshot and video
- `videos` — upserted on ingestion, enriched later (Epic 5)

**Key relationships**:
```
User 1:N FeedSnapshot 1:N FeedItem N:1 Video
```
...
|--------|------|-------------|------|-----------|
| POST | `/api/feeds` | Submit feed capture from extension | Clerk JWT or Extension Token | 10/hour/user |
| GET | `/api/feeds` | List user's feed snapshots (paginated) | Clerk JWT | None |
| GET | `/api/feeds/[id]` | Get single feed with items and video data | Clerk JWT | None |
...
| 404 | `Not Found` | Feed not found or not owned by user |
...
|------|-------------|-----------|-------------|-------------|
| T4.1 | POST /api/feeds handler with auth | `src/app/api/feeds/route.ts` | backend-solutions-engineer | T1.3, T3.1 |
| T4.2 | Zod validation schema | `src/lib/feed-schema.ts` | backend-solutions-engineer | T4.1 |
| T4.3 | Feed snapshot + item creation (Prisma transaction) | `src/app/api/feeds/route.ts` | backend-solutions-engineer | T4.2 |
| T4.4 | Rate limiting middleware | `src/lib/rate-limit.ts` | backend-solutions-engineer | T1.4 |
| T4.5 | GET /api/feeds (list) | `src/app/api/feeds/route.ts` | backend-solutions-engineer | T4.1 |
| T4.6 | GET /api/feeds/[id] (detail) | `src/app/api/feeds/[id]/route.ts` | backend-solutions-engineer | T4.1 |
| T4.7 | Error handling + logging | `src/app/api/feeds/route.ts` | backend-solutions-engineer | T4.1-T4.6 |
| T4.8 | Integration tests | `src/app/api/feeds/__tests__/*` | tdd-test-engineer | T4.1-T4.7 |
...
- **Integration tests**: Full POST flow (authenticate -> validate -> insert -> verify DB state)
- **Integration tests**: GET list with pagination
- **Integration tests**: GET detail with nested video data
- **Integration tests**: 401 on unauthenticated request
